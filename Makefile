...
run-api:
	docker run -d --env GOAL_ENV_FILE=.env --name $(API_CONTAINER_NAME) -p 8080:8080 $(IMAGE_NAME)

run-admin:
	docker run -d --env GOAL_ENV_FILE=.env --name $(ADMIN_CONTAINER_NAME) -p 8081:8080 $(IMAGE_NAME)
